...
Usage:
  $0 [--language go] [--version <tag>] [--no-save] [--no-cache]
  $0 --help

Options:
  --language, -l   Extension language. PolicyGuard ships Go only.
...
Output:
  Image  policyguard-extension-<language>:<version>
  Tar    policyguard-extension-<language>-<version>.tar   (unless --no-save)
EOF
}
...
IMAGE="policyguard-extension-${LANGUAGE}:${VERSION}"
TAR="$PROJECT_DIR/policyguard-extension-${LANGUAGE}-${VERSION}.tar"
