...
#
# Usage:
#   ./scripts/use-chain.sh [local] <chain> [go]
...
# PolicyGuard implements the extension in Go only; the scaffold's Python and
# TypeScript variants are not part of this project.
LANGUAGES="go"

usage() { echo "usage: $0 [local] <chain> [go] | --list | --help" >&2; }
...
Arguments:
  ...
  [language]   Optional: go (the only language this project ships). Rewrites LANGUAGE= in .env.
